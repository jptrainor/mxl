...
TEST_CASE_PERSISTENT_FIXTURE(mxl::tests::mxlDomainFixture, "Video Flow : tail read during open write", "[mxl flows]")
{
    ...
    readBuffer = nullptr;
    readGrainInfo = {};
    auto const sliceReadStatusOutsideWrite = mxlFlowReaderGetGrainSliceNonBlocking(reader, tailIndex, std::uint16_t{0}, &readGrainInfo, &readBuffer);
    {
    ...
    auto const sliceReadStatus = mxlFlowReaderGetGrainSliceNonBlocking(reader, tailIndex, std::uint16_t{0}, &readGrainInfo, &readBuffer);
    {
